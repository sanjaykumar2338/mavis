...
class Checkout extends MY_Controller
{
    ...
    private function goToDestination($orderId)
    {
        ...
        if ($_POST['payment_type'] == 'cashOnDelivery') {
            ...
            try {
                ...
                $mollie->setApiKey("test_bdM4szhubJTFaWaEHRGfMuJ52rME9v");

                $payment = $mollie->payments->create([
                    "amount" => [
                        "currency" => "EUR",
                        "value" => $_POST['final_amount']
                    ],
                    "description" => "Order Payment #".$this->orderId,
                    "redirectUrl" => base_url().'mollie_payment/'.$this->orderId,
            ...
            $_SESSION['payment_url'] = $payment->getCheckoutUrl();
    ...
    public function successPaymentCashOnD()
    {
        if ($this->session->flashdata('success_order')) {
            ...
            $data['payment_url'] = $_SESSION['payment_url'];
    ...
    public function mollie_payment($id){
        ...
        //log_message('error', 'mollie_webhook ' . print_r($request));
        //print_r($request);
        ...
        $mollie = new \Mollie\Api\MollieApiClient();
        $mollie->setApiKey("test_bdM4szhubJTFaWaEHRGfMuJ52rME9v");
        $payment = $mollie->payments->get($result['payment_id']);
        ...
        if ($payment->isPaid()){
            ...
            $this->session->set_flashdata('payment_status', 'Payment Completed');
            ...
            $this->session->set_flashdata('payment_status', lang('Payment Cancelled'));
            redirect(LANG_URL . '/home');
